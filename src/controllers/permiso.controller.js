...
const create  = async (req, res) => { try { res.status(201).json(await svc.create(req.body.nombre)); } catch(e) { handleError(res,e); } };
const update  = async (req, res) => { try { res.json(await svc.update(req.params.id, req.body.nombre)); } catch(e) { handleError(res,e); } };
